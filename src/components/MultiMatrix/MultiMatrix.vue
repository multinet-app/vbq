...
export default Vue.extend({
  ...
  methods: {
    ...
    sort(order: string): void {
      const nodeIDs = this.schemaNetwork.nodes.map((node: Node) => node.id);
    ...
    isQuantitative(varName: string): boolean {
      const uniqueValues = [
        ...new Set(
          this.schemaNetwork.nodes.map((node: Node) =>
            parseFloat(node[varName]),
          ),
        ),
      ];
    ...
    sortObserver(type: string, isNode = false): number[] {
      ...
      if (
        ...
        const sortableNetwork = reorder
          .graph()
          .nodes(this.schemaNetwork.nodes)
          .links(links)
          .init();
        ...
        order = range(this.schemaNetwork.nodes.length).sort(
          (a, b) =>
            this.schemaNetwork.nodes[b][type] -
            this.schemaNetwork.nodes[a][type],
        );
      } else if (isNode === true) {
        order = range(this.schemaNetwork.nodes.length).sort((a, b) =>
          this.schemaNetwork.nodes[a].id.localeCompare(
            this.schemaNetwork.nodes[b].id,
          ),
        );
        order = range(this.schemaNetwork.nodes.length).sort(
          (a, b) =>
            Number(this.schemaNetwork.nodes[b].neighbors.includes(type)) -
            Number(this.schemaNetwork.nodes[a].neighbors.includes(type)),
        );
